                  ...
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="text-left border-b bg-gray-50">
                        <th className="py-2 px-3 sticky left-0 bg-gray-50 z-10">지표</th>
                        <th className="py-2 px-3">컷오프(샘플)</th>
                        <th className="py-2 px-3">요약(Overall)</th>
                        <th className="py-2 px-3">상태</th>
                        <th className="py-2 px-3">Worst 이벤트</th>
                        {matrix.eventCols.map((c) => (
                          <th key={c.id} className="py-2 px-3 whitespace-nowrap">{c.name}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {matrix.rows.map((r) => {
                        ...
                        const overallHint = (() => {
                          if (ind.riskDirection === "bool-worse") return "무반응 비율(Overall)";
                          if (overallMode === "mean") return "이벤트 값 평균";
                          if (overallMode === "max") return "이벤트 값 최대";
                          if (overallMode === "min") return "이벤트 값 최소";
                          return ind.riskDirection === "lower-worse" ? "최악값(최소)" : "최악값(최대)";
                        })();
                        ...
                        const worstName = worst
                          ? (matrix.eventCols.find((x) => x.id === worst.eventId)?.name ?? worst.eventId)
                          : "-";

                        return (
                          <tr key={ind.id} className="border-b align-top">
                            <td className="py-3 px-3 sticky left-0 bg-white z-10">
                              <div className="font-medium">{ind.label}</div>
                              <div className="text-xs text-gray-500 mt-1">{ind.logs}</div>
                            </td>

                            <td className="py-3 px-3">
                              <div className="font-medium">{formatCutoff(ind)}</div>
                              <div className="text-xs text-gray-500 mt-1">{NORMS[ind.normKey]?.note ?? ""}</div>
                            </td>

                            <td className="py-3 px-3">
                              <div className="font-semibold">{overallText}</div>
                              <div className="text-xs text-gray-500 mt-1">{overallHint}</div>
                            </td>

                            <td className="py-3 px-3">
                              <span className={`inline-flex items-center px-2 py-1 rounded-lg border text-xs ${pillClass(level)}`}>
                                {statusText}
                              </span>
                            </td>

                            <td className="py-3 px-3 whitespace-nowrap">
                              <div className="font-medium">{worstName}</div>
                              <div className="text-xs text-gray-500 mt-1">
                                {worst ? `값: ${ind.format(worst.value)}` : ""}
                              </div>
                            </td>

                            {matrix.eventCols.map((c) => {
                              const v = r.values[c.id];
                              const cell = v === undefined ? "-" : ind.format(v);
                              return (
